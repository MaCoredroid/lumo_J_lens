...
def visibility_audit(
    *,
    ...
    for subject in concept_subjects(concepts, instance_id):
        ...
        channel_evidence = {
            name: C1.exposure_evidence(text, str(subject["target"]), forms, tokenizer)
            for name, text in channels.items()
        }
        rendered_evidence = C1.exposure_evidence(
            rendered, str(subject["target"]), forms, tokenizer
        )
