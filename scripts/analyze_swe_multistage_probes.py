...
def validate_prompt_bundle(
    ...
    *,
    ...
    return {
        ...
        "hidden_gold_eligible_prompt_count": sum(
            int(prompt["hidden_gold_eligible"]) for prompt in result
        ),
        "explicit_control_prompt_count": sum(
            int(not prompt["hidden_gold_eligible"]) for prompt in result
        ),
    }
...
def build_analysis(
    *,
    ...
    hidden_gold_eligible_prompt_count = prompt_contract[
        "hidden_gold_eligible_prompt_count"
    ]
    ...
    return {
        ...
        "gold_probe_status": (
            ...
            if hidden_gold_eligible_prompt_count
            else "no_hidden_gold_eligible_prompts"
        ),
        "numerical_status": numerical_status,
        "interpretation_contract": {
            "n1_is_descriptive_only": task_count == 1,
            "task_count": task_count,
            "hidden_gold_eligible_prompt_count": hidden_gold_eligible_prompt_count,
            "explicit_control_prompt_count": prompt_contract[
                "explicit_control_prompt_count"
            ],
            "primary_gold_metric": (
